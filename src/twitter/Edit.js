			...
			<div className={`etfTwitterFeed`}>
				{type ? <>
					...
					<Placeholder icon={twitterIcon('#03A9F4')} instructions={__("Choose a Feed type to get started.", "easy-twitter")} label={__("Choose a Feed Type", "easy-twitter")}>
						<BtnGroupPro value={type} onChange={val => setAttributes({ type: val })} options={types} proValues={['tweet', 'video', 'post', 'tag']} {...premiumProps} setIsProModalOpen={setAboutProOpen} />
					</Placeholder>}
			</div>
		</div>
	</>;
